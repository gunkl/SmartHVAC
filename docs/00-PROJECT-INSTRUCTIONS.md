...
| Why does pred_indoor sometimes track actual indoor (delta ≈ 0)? | Thermal model empty → physics gate falsy → constant fallback; 30-min refresh cycle restores after restart (Issue #137). Four root causes: restart, fresh install, sensor unavailable, outdoor≈indoor off-day. | [Chart Log Spec](chart-log-spec.md#regression-decision-tree) |
